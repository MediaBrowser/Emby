...
{
    public class LegacyHdHomerunCommand
    {
        ...
        public LegacyHdHomerunCommand(ISocketFactory socketFactory)
        {
            _socketFactory = socketFactory;
        }

        ~LegacyHdHomerunCommand()
        {
            StopStreaming();
        }
        ...
        public async Task<bool> CheckTunerAvailability(IpAddressInfo remoteIp, int tuner, CancellationToken cancellationToken)
        {
            ...
            var response = await tcpClient.ReceiveAsync().WithCancellation(cancellationToken);
            string returnVal;
        ...
        public async Task StartStreaming(IpAddressInfo remoteIp, IpAddressInfo localIp, int localPort, string url, int numTuners, CancellationToken cancellationToken)
        {
            ...
            for (int i = 0; i < numTuners; ++i)
            {
                ...
                await tcpClient.ReceiveAsync().WithCancellation(cancellationToken);
                // parse response to make sure it worked
                if (!ParseReturnMessage(response.Buffer, response.ReceivedBytes, out returnVal))
                {
                    ReleaseLockkey(tcpClient);
                    continue;
                }
                ...
                response = await tcpClient.ReceiveAsync().WithCancellation(cancellationToken);
                // parse response to make sure it worked
                if (!ParseReturnMessage(response.Buffer, response.ReceivedBytes, out returnVal))
                {
                    ReleaseLockkey(tcpClient);
        ...
        private void ReleaseLockkey(IUdpSocket tcpClient)
        {
            var releaseTarget = CreateSetMessage(_activeTuner, "target", "none", _lockkey);
            tcpClient.SendAsync(releaseTarget, releaseTarget.Length, new IpEndPointInfo(_remoteIp, HdHomeRunPort), CancellationToken.None);
            var response = tcpClient.ReceiveAsync();
            var releaseKeyMsg = CreateSetMessage(_activeTuner, "lockkey", "none", _lockkey);
            _lockkey = null;
            tcpClient.SendAsync(releaseKeyMsg, releaseKeyMsg.Length, new IpEndPointInfo(_remoteIp, HdHomeRunPort), CancellationToken.None);
            response = tcpClient.ReceiveAsync();
        }
