...
namespace MediaBrowser.Server.Implementations.PlayTo.Helpers
{
    internal sealed class SsdpHelper
    {
        ...
        private const string SSDP_RENDERER = "M-SEARCH * HTTP/1.1\r\n" +
        ...
        {
            return Encoding.UTF8.GetBytes(string.Format(SSDP_RENDERER, mx));
        }
        ...
        public static Uri ParseSsdpResponse(string data)
        {
            try
            {
                var res = (from line in data.Split(new char[] { '\r', '\n' })
                           where line.ToLowerInvariant().StartsWith("location:")
                           select line).FirstOrDefault();

                if (res != null)
                    return new Uri(res.Substring(9).Trim());
                return null;
            }
            catch
            {
        ...
        public static string ParseSsdpEvent(string data)
        {
            try
            {
                var sid = (from line in data.Split(new char[] { '\r', '\n' })
