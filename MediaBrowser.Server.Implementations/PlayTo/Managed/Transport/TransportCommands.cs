...
namespace MediaBrowser.Server.Implementations.PlayTo.Managed.Transport
{
    ...
    public class TransportCommands
    {
        ...
        public static TransportCommands CreateFromXml(XDocument document)
        {
            var command = new TransportCommands();

            var actionList = document.Descendants(uPnpNamespaces.svc + "actionList");

            foreach (XElement container in actionList.Descendants(uPnpNamespaces.svc + "action"))
            {
            ...
            foreach (XElement container in stateValues.Elements(uPnpNamespaces.svc + "stateVariable"))
            {
        ...
        internal string BuildPost(ServiceAction action, string xmlNamespace)
        {
            var stateString = string.Empty;
            foreach (Argument arg in action.ArgumentList)
            {
        ...
        internal string BuildPost(ServiceAction action, string xmlNamesapce, object value, string commandParameter = "")
        {
            var stateString = string.Empty;
            foreach (Argument arg in action.ArgumentList)
            {
        ...
        internal string BuildSearchPost(ServiceAction action, string xmlNamesapce, object value, string commandParameter = "")
        {
            var stateString = string.Empty;
            foreach (Argument arg in action.ArgumentList)
            {
        ...
        internal string BuildPost(ServiceAction action, string xmlNamesapce, object value, Dictionary<string, string> dictionary)
        {
            var stateString = string.Empty;
            foreach (Argument arg in action.ArgumentList)
            {
        ...
        {
            var state = this.StateVariables.Where(a => a.Name == argument.RelatedStateVariable).FirstOrDefault();            
            if (state != null)
            {
                var sendValue = state.AllowedValues.Where(a => a == commandParameter).FirstOrDefault();
                if (sendValue == null)
                    sendValue = state.AllowedValues.FirstOrDefault();
                if (sendValue == null)
                    sendValue = value;
                return string.Format("<{0} xmlns:dt=\"urn:schemas-microsoft-com:datatypes\" dt:dt=\"{1}\">{2}</{0}>", argument.Name,(state.DataType == null) ? "string": state.DataType, sendValue);
            }
            return string.Format("<{0}>{1}</{0}>", argument.Name, value);
        }
        
        private static string CommandBase = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\r\n" +
